...
namespace Aimrank.Cluster.Api.Middleware
{
    public class ExceptionsHandlerMiddleware : IMiddleware
    {
        ...
        private static ProblemDetails MapExceptionToProblemDetails(Exception exception)
            => exception switch
            {
                NotFoundException ex => Problem(ex.Message, StatusCodes.Status404NotFound),
                ClusterException ex => Problem(ex.Message, StatusCodes.Status409Conflict),
